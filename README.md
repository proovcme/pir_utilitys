...
Публичная редакция расчёта стоимости проектных работ по ФГИС ЦС:

```bash
npm run build:public-pir
```

Она собирается отдельно в `dist-public/` с базовым путем `/pircalc/`. В нее не входят коммерческие ставки, ПП №87, подряд, шаблоны, портфель и API Locia. После сборки скрипт автоматически проверяет эту границу по маркерам приватных модулей.
