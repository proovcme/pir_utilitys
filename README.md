...
Публичная редакция форм 1П/2П/3П для расчёта стоимости проектных работ:

```bash
npm run build:public-pir
```

Она собирается отдельно в `dist-public/` с базовым путем `/pircalc/`. Форма 2П использует нормативы ФГИС ЦС, форма 3П — калькуляцию трудозатрат по Приказу № 707/пр, форма 1П автоматически сводит рассчитанные документы. В публичную сборку не входят коммерческие ставки, ПП №87, подряд, шаблоны, портфель и API Locia. После сборки скрипт автоматически проверяет эту границу по маркерам приватных модулей.
